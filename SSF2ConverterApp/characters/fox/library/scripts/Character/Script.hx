// API Script for fox — converted from SSF2
...
function ssf2_initialize() {
	Engine.log("Fox initialized.");
	self.addEventListener(SSF2Event.CHAR_KO_DEATH, self.resetChargeOnDeath, { persistent: true });
	return;
}
...
function resetChargeOnDeath(arg0) {
	self.updateAnimationStatsMetadata(/* TODO: setGlobalVariable */ "FoxDSpecCharge", 0);
	self.updateAnimationStatsMetadata(/* TODO: setGlobalVariable */ "FoxDSpecFrame", 0);
	self.updateAnimationStatsMetadata(/* TODO: setGlobalVariable */ "FoxDSpecDoIt", false);
	return;
}
