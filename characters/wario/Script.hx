...
function initialize() {
         ...
         this.redoTimer = new FrameTimer(120);
         this.colTrans = new ColorTransform();
         this.tintClock = -1;
      
}

function updateFartLevel() {

         if(this.fartTick > 110 * 30)
         {
            this.fart = 3;
         }
         else if(this.fartTick > 55 * 30)
         {
            this.fart = 2;
         }
         else if(this.fartTick > 15 * 30)
         {
            this.fart = 1;
         }
         else
         ...
         if(// TODO: SSF2API.isDebug() && // TODO: SSF2API.getGlobalVar("waftdebugpower") > 0)
         {
            this.fart = // TODO: SSF2API.getGlobalVar("waftdebugpower");
         }
      
}

function getFartLevel() {

         return this.fart;
      
}

function resetFartTick() {
...
function createFartEffect() {
         ...
         if(this.fartEffect)
         {
            if(this.fartEffect.parent)
            {
               this.fartEffect.parent.removeChild(this.fartEffect);
            }
         }
         if(!self.isStandby())
         {
...
function update() {
         ...
         self.updateFartLevel();
         if(this.fartEffect)
         {
            if(self.getFartLevel() == 1)
            {
               this.fartEffect.gotoAndStop("level1");
            }
            else if(self.getFartLevel() == 2)
            {
               this.fartEffect.gotoAndStop("level2");
            }
            else if(self.getFartLevel() == 3)
            {
               this.fartEffect.gotoAndStop("level4");
            }
            else
            {
               this.fartEffect.gotoAndStop("level0");
            }
            this.fartEffect.scaleX = self.getMC().scaleX;
            ...
            {
               this.fartEffect.x = self.getX();
               this.fartEffect.y = self.getY();
            }
            if(self.getMC().visible)
            {
               this.fartEffect.visible = true;
            }
            else
            {
               this.fartEffect.visible = false;
            }
         }
         else
         {
            self.createFartEffect();
         }
         this.redoTimer.tick();
         ...
         if(!self.getIntangibility() && !self.getInvincibility())
         {
            if(self.getFartLevel() == 3)
            {
               if(this.tintClock <= 4)
               {
                  this.colTrans.redOffset = 50 - this.tintClock * 7.5;
                  this.colTrans.greenOffset = 50 - this.tintClock * 7.5;
                  this.colTrans.blueOffset = 0;
                  this.colTrans.redMultiplier = 0.75 + this.tintClock * 0.0375;
                  this.colTrans.greenMultiplier = 0.75 + this.tintClock * 0.0375;
                  this.colTrans.blueMultiplier = 0.75 + this.tintClock * 0.0375;
               }
               else
               {
                  this.colTrans.redOffset = 20;
                  this.colTrans.greenOffset = 20;
                  this.colTrans.blueOffset = 0;
                  this.colTrans.redMultiplier = 0.9;
                  this.colTrans.greenMultiplier = 0.9;
                  this.colTrans.blueMultiplier = 0.9;
               }
               self.getMC().transform.colorTransform = this.colTrans;
               this.tintClock = (this.tintClock + 1) % 7;
            }
            else if(self.getFartLevel() == 2)
            {
               if(this.tintClock <= 2)
               {
                  this.colTrans.redOffset = 30 - this.tintClock * 15;
                  this.colTrans.greenOffset = 30 - this.tintClock * 15;
                  this.colTrans.blueOffset = 0;
                  this.colTrans.redMultiplier = 0.85 + this.tintClock * 0.075;
                  this.colTrans.greenMultiplier = 0.85 + this.tintClock * 0.075;
                  this.colTrans.blueMultiplier = 0.85 + this.tintClock * 0.075;
               }
               else
               {
               ...
               self.getMC().transform.colorTransform = this.colTrans;
               this.tintClock = (this.tintClock + 1) % 10;
            }
            else if(self.getFartLevel() < 2)
            {
               this.tintClock = -1;
...
function smash64(param1:Array<Dynamic>) {
