...
// ── Yoshi-specific overrides (ported from SSF2 YoshiExt.as) ──
...
function initialize() {
         ...
         self.addEventListener(SSF2Event.CHAR_SHIELD_HIT,this.shieldEffect,{"persistent":true});
         self.addEventListener(SSF2Event.CHAR_POWER_SHIELD_HIT,this.shieldPowerEffect,{"persistent":true});
...
function update() {
         ...
         {
            self.getStanceMC().shieldframe.gotoAndStop(Math.ceil(self.getShieldPower() * 100) + 1);
         }
         if(self.getState() == CState.SHIELD_DROP)
         {
            if(self.getStanceMC().shieldframe.alpha == 1)
            {
               self.getStanceMC().shieldframe.alpha = self.getStanceMC().shieldframe.alpha - 0.15;
               self.getStanceMC().shieldframe.scaleX = self.getStanceMC().shieldframe.scaleX - 0.05;
               self.getStanceMC().shieldframe.scaleY = self.getStanceMC().shieldframe.scaleY - 0.05;
               self.getStanceMC().shieldframe.y = self.getStanceMC().shieldframe.y + 1;
            }
         }
